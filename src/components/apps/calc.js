...
export class Calc extends Component {
    ...
    handleCommands = (command, rowId) => {
        ...
        switch (main) {        
            ...
            // default:
            //     result =this.isValid(command)? parser.evaluate(command) : "Invalid Arthimetic Expression";
        }
    ...
    render() {
        return (
            <div className="h-full w-full bg-ub-drk-abrgn text-ubt-grey opacity-100 p-1 float-left font-normal">
                ...
                <div>[ type "exit" to exit, "clear" to clear. ]</div>
            <div className="text-white text-sm font-bold" id="calculator-body">
                {this.state.terminal}
            </div>
            </div>
        )
